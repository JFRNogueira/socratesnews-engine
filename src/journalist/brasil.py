...
def brasil():
    ...
    with col1:
        ...
        if 'brasil_themes' in st.session_state:
            
            # Renderizar o editor de dados depois das métricas
            st.session_state['brasil_themes_selection'] = st.data_editor(
                st.session_state['brasil_themes_selection'] if 'brasil_themes_selection' in st.session_state else st.session_state['brasil_themes'], 
                column_config={
            ...
            if 'brasil_themes_selection' in st.session_state and (st.session_state['brasil_themes_selection']['create'] == True).sum() > 0:
                if st.button("Buscar fontes para notícias"):
                    GoogleNewsCluster(['Brasil']).get_all_references()
                
            if any(f'brasil_news_{i}_image_text' in st.session_state for i in range(1, 10)):
                if st.button(f"Gerar todas as notícias"):
                    for i in range(1, 20):
                        if f'brasil_news_{i}_image_text' in st.session_state:
                            selection = st.session_state[f'brasil_news_{i}_reference_news_selected']
                            st.session_state[f'brasil_news_{i}_final'] = WriterNews('Brasil', 
